...
5.  The "sample.abdera.jaxrs [master]" repo should appear in the view
...
1. Right-click on sample.abdera.jaxrs/build.gradle
...
6.  This will create 3 projects in Eclipse: abdera-jaxrs, abdera-jaxrs-application, and abdera-jaxrs-wlpcfg

:star: *Note:* If you did not use Eclipse/WDT to clone the git repository, follow from step 3, but navigate to the cloned repository directory rather than pasting its name in step 4.

###### Run Maven install

1. Right-click on abdera-jaxrs/pom.xml
...
For the purposes of this sample, we will create the Liberty server (step 3 in the wasdev.net instructions) a little differently to create and customize a Runtime Environment that will allow the server to directly use the configuration in the `abdera-jaxrs-wlpcfg` project.
...
3. If the `abdera-jaxrs-wlpcfg` directory is not listed as a User Directory, we need to add it:
    1. Click New
    2. Select the `abdera-jaxrs-wlpcfg` project, if not exist `use an external user directory` in the `Rep_working_dir/abdera-jaxrs-wlpcfg`
    3. Select *Finish*, *OK*, *Finish*
4. Right-click on the `abdera-jaxrs-wlpcfg` user directory listed under the target Runtime Environment in the Runtime Explorer view, and select *New Server*.
...
1.  Select the `abdera-jaxrs-application` project
2.  Right-click -> *Run As... -> Run On Server*
3.  Select the appropriate server (as created above) and select *Finish*
4.  Confirm web browser opens on "http://localhost:9081/abdera-jaxrs-application/" with 3 hyperlinks to run samples
